...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
	...
	glGetIntegerv(GL_CLIP_ORIGIN, reinterpret_cast<GLint*>(&last_clip_origin)); // Support for GL 4.5's glClipControl(GL_UPPER_LEFT)
	...
	for (auto n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		for (auto cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			{
				const auto clip_rect = ImVec4(pcmd->ClipRect.x - pos.x, pcmd->ClipRect.y - pos.y, pcmd->ClipRect.z - pos.x,
				                          pcmd->ClipRect.w - pos.y);
				if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
				{
					// Apply scissor/clipping rectangle
					if (clip_origin_lower_left)
						glScissor(static_cast<int>(clip_rect.x), static_cast<int>(fb_height - clip_rect.w), static_cast<int>(clip_rect.z - clip_rect.x),
						          static_cast<int>(clip_rect.w - clip_rect.y));
					else
						glScissor(static_cast<int>(clip_rect.x), static_cast<int>(clip_rect.y), static_cast<int>(clip_rect.z), static_cast<int>(clip_rect.w));
